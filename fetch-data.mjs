...
function whenFrom(iso) {
	...
	const h = Math.max(0, Math.round((Date.now() - then) / 36e5));
	...
	const OPEN = open
		.map((row) => {
			...
			return {
				...
				reviewers: [],
				status,
				when: "updated " + whenFrom(row.dateIso),
				ts
			};
		})
		.sort((a, b) => b.ts - a.ts);
...
${HELPERS}
	...
	window.DASH = { SQUAD: SQUAD, REPOS: REPOS, MERGED: MERGED, OPEN: OPEN, AREAS: AREAS, TOTALS: TOTALS, person: person, sprinkleBurst: sprinkleBurst };
})();
`;
}
